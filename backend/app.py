...
def semantic_search(query: str) -> dict:
    ...
    for i, sim in enumerate(sims):
        ...
        if s > 0.55:  # порог: пропускает ~1900 проектов из 2210
...
def format_organizations_v2(scored_records: list[tuple], top_n: int = 5) -> list[dict]:
    ...
    for oid, d in org_data.items():
        ...
        d["top_projects"] = [{
            ...
            "abstract_full":    (r.get("abstract") or ""),
            ...
            "org_name_full":    r.get("org_name_full", ""),
            "org_slug":         r.get("org_slug", ""),
            "org_logo":         r.get("org_logo", ""),
            "org_website":      r.get("org_website", ""),
...
def generate_org_reasoning(query: str, organizations: list[dict]) -> list[dict]:
    ...
    system_prompt = """Ты — эксперт-аналитик платформы МОСНАУКА. Твоя задача — объяснить бизнес-заказчику,
почему каждая из найденных организаций может решить его задачу.

Напиши для каждой организации 2-4 предложения в стиле рекомендательной записки:

1. Начни с КОНКРЕТНОГО опыта: не "имеет опыт", а "разработала технологию X" / "создала прототип Y" / "провела испытания Z"
2. Объясни ЦЕННОСТЬ для заказчика: "что позволяет сократить сроки" / "есть готовый задел" / "результаты можно масштабировать"
3. Если есть смежный, а не прямой опыт — честно скажи: "имеет опыт в смежной области X, методы применимы к задаче заказчика"

ЗАПРЕЩЕНО:
- Общие фразы: "является ведущим", "обладает компетенциями", "имеет значительный опыт"
- Перечисление статистики: "3 проекта, бюджет X млн"
- Копирование названий проектов целиком

СТИЛЬ: как если бы опытный брокер НИОКР объяснял заказчику за чашкой кофе — конкретно, полезно, без канцелярита.

Верни JSON: [{"org": "Название", "reasoning": "Текст"}]
Только JSON, без markdown."""

    user_prompt = f"""Задача заказчика: «{query}»

Найденные организации и их релевантные проекты:
{org_contexts}

Для каждой организации — почему именно она может помочь заказчику?"""
...
def rerank_with_ai(query: str, scored_records: list[tuple]) -> list[tuple]:
    """Claude Haiku отсеивает явно нерелевантные записи из топ-15.
    
    Мягкая фильтрация: убирает только явный мусор.
    Гарантирует минимум 6 проектов в результате.
    """
...
ПРАВИЛО: Одно слово в разных областях = РАЗНЫЕ смыслы. Примеры:
- «доставка грузов» (логистика) ≠ «доставка лекарств в клетку» (биохимия)
- «транспорт грузов» ≠ «внутриклеточный транспорт» (молекулярная биология)
- «защита данных» (ИТ) ≠ «защита растений» (агрономия)

Включай: прямо по теме + смежные инженерные/научные проекты.
Исключай: проекты где ключевое слово используется в ДРУГОЙ научной области.

Ответь ТОЛЬКО номерами через запятую."""},
                    {"role": "user", "content": f"Запрос: «{query}»\n\nПроекты:\n{titles_list}\n\nНомера проектов, ТЕМАТИЧЕСКИ связанных с запросом:"},
                ],
                "max_tokens": 60,
                "temperature": 0.0,
            },
        ...
        # Оставляем релевантные проекты + все не-проекты
        reranked = [
            (s, r) for s, r in scored_records
            if r["type"] != "project" or r["id"] in relevant_ids
        ]
        
        # Гарантируем минимум 6 проектов: если мало — добираем из оригинала
        proj_in_reranked = [(s, r) for s, r in reranked if r["type"] == "project"]
        if len(proj_in_reranked) < 6:
            existing_ids = {r["id"] for _, r in proj_in_reranked}
            for s, r in scored_records:
                if r["type"] == "project" and r["id"] not in existing_ids:
                    reranked.append((s, r))
        ...
        print(f"🔄 Rerank: {len(top_to_rerank)} → {len(valid_indices)} relevant (Claude: {answer})")
        return reranked
